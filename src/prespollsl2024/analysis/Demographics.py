import os
from gig import GIGTable, Ent, EntType
from utils import Log, JSONFile

log = Log('Demographics')

class Demographics:
    GT_ELECTION = GIGTable('government-elections-presidential', 'regions-ec', '2019')
    ...
    def analyze(group_to_ent_id_list):
        ...
        for group, electors in sorted(group_to_electors.items(), key=lambda x: x[1], reverse=True):
...
if __name__ == '__main__':
    ...
    group_to_ed_id_list = Demographics.get_group_to_ent_id_list(Demographics.get_ed_id_to_ent())
    Demographics.analyze(group_to_ed_id_list)
    JSONFile(os.path.join('data-analysis', 'group_to_ed_id_list.json')).write(group_to_ed_id_list)