...
from prespollsl2024.ec.ForParty import ForParty
...
class PDResult1:
    ...
    def to_dict_compact(self):
        return {
            ...
            "party_to_votes": ForParty.to_dict_compact(self.by_party),
    ...
    def from_dict(d):
        return PDResult1(
            ...
            pd_code=d["pd_code"],
            pd_name=d["pd_name"],
            by_party=[ForParty.from_dict(x) for x in d["by_party"]],
    ...
    @staticmethod
    def from_file(json_file_path: str) -> 'PDResult1':
        d = JSONFile(json_file_path).read()
        ec_data = PDResult1.from_dict(d)
        log.debug(f'Loaded from {json_file_path}')
        return ec_data

    @staticmethod
    def list_for_dir(dir_path: str) -> list['PDResult1']:
        ec_data_list = []
        for file_name in os.listdir(dir_path):
            if not file_name.endswith(".json"):
                continue
            ec_data = PDResult1.from_file(os.path.join(dir_path, file_name))
            ec_data_list.append(ec_data)
        log.info(f'Loaded {len(ec_data_list)} PDResult1 from {dir_path}')
        return ec_data_list

    @staticmethod
    def list_from_test() -> list['PDResult1']:
        return PDResult1.list_for_dir(PDResult1.DIR_DATA_TEST)

    @staticmethod
    def list_from_prod() -> list['PDResult1']:
        return PDResult1.list_for_dir(PDResult1.DIR_DATA_PROD)

    @staticmethod
    def store_list_to_dir(ec_data_list: list['PDResult1'], dir_path: str):
        ...
        log.info(f'Stored {len(ec_data_list)} PDResult1 to {dir_path}')

    @staticmethod
    def store_list_to_json_compact(
        ec_data_list: list['PDResult1'], json_file_path: str
    ):
    ...
    @staticmethod
    def build_tsv(ec_data_list: list['PDResult1'], tsv_file_path: str):
        data_list = []
        for ec_data in ec_data_list:
